...
namespace Witivio.Copilot4Researcher.Features.ClinicalTrials.Cards
{
    public class ClinicalTrialCardRenderer
    {
        ...
        private static List<AdaptiveElement> GenerateContactsColumns(List<ClinicalTrialContact> contacts)
        {
        ...
        private static Uri GetStatusBackground(string status)
        {
            switch (status)
            {
                ...
                case "RECRUITING":
                    return new Uri(CardIcons.GreenBg);
                case "NOT_YET_RECRUITING":
                    return new Uri(CardIcons.YellowBg);
                case "UNKNOWN":
                    return new Uri(CardIcons.GreyBg);
                default:
