...
namespace Witivio.Copilot4Researcher.Features.ClinicalTrials
{
    public class ClinicalTrialsClient : IClinicalTrialsClient
    {
        ...
        public async Task<IEnumerable<ClinicalTrial>> SearchAsync(ClinicalTrialSearchQuery query)
        {
            ...
            return result.Studies.Select(s => new ClinicalTrial
            {
                ...
                Contacts = s.ProtocolSection?.ContactsLocationsModule?.CentralContacts?.Select(c => new ClinicalTrialContact
                {
                    Name = c.Name,
                    Email = c.Email,
                    Phone = c.Phone
